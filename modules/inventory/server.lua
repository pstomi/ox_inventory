...
function Inventory.SlotWeight(item, slot, ignoreCount)
    ...
    if item.hash == `WEAPON_FIREEXTINGUISHER` or item.hash == `WEAPON_PETROLCAN` or item.hash == `WEAPON_HAZARDCAN` or item.hash == `WEAPON_FERTILIZERCAN` or item.hash == `WEAPON_FLAMETHROWER` then
...
function Inventory.AddItem(inv, item, count, metadata, slot, cb)
	...
	if toSlotType == 'number' then
		...
		if invokingResource then
			lib.logger(inv.owner, 'targyAtadas', ('"%s" script átadott %sdb %s tárgyat neki: "%s"'):format(invokingResource, count, item.name, inv.label))
		end
...
function Inventory.RemoveItem(inv, item, count, metadata, slot, ignoreTotal, strict)
	...
	if count > 0 then
		...
		if removed > 0 then
			...
			if invokingResource then
				lib.logger(inv.owner, 'targyElvetel', ('"%s" script eltávolított %sdb %s tárgyat töle: "%s"'):format(invokingResource, removed, item.name, inv.label))
			end
...
exports('CreateDropFromPlayer', function(playerId)
	...
	local dropId = generateInvId()
	local inventory = Inventory.Create(dropId, 'Föld', 'drop', playerInventory.slots, playerInventory.weight, playerInventory.maxWeight, false, table.clone(playerInventory.items))
...
local function dropItem(source, playerInventory, fromData, data)
	...
	local inventory = Inventory.Create(dropId, 'Föld', 'drop', shared.dropslots, toData.weight, shared.dropweight, false, {[data.toSlot] = toData})
	...
	if server.loglevel > 0 then
		lib.logger(playerInventory.owner, 'targyKidobas', ('license:%s %sdb %s tárgyat kidobott ebböl az inventorybol %s'):format(playerInventory.owner, data.count, toData.name, playerInventory.label),
			playerInventory.owner, playerInventory.label, toData.name
		)
	end
			...
			if toData and ((toData.name ~= fromData.name) or not toData.stack or (not table.matches(toData.metadata, fromData.metadata))) then
				...
				if not sameInventory then
					if (toWeight <= toInventory.maxWeight and fromWeight <= fromInventory.maxWeight) then
						...
						if server.loglevel > 0 then
							lib.logger(playerInventory.owner, 'targyAthelyezes', ('license:%s %sdb %s tárgy lett ki lett cserélve %s inventoryból %s inventoryba %sdb %s tárgyra'):format(playerInventory.owner, fromData.count, fromData.name, fromInventory.owner and fromInventory.label or fromInventory.id, toInventory.owner and toInventory.label or toInventory.id, toData.count, toData.name), fromInventory.label, toInventory.label, fromInventory.owner, toInventory.owner, toData.name)
						end
			...
			elseif toData and toData.name == fromData.name and table.matches(toData.metadata, fromData.metadata) then
				...
				if fromInventory.type == 'container' or sameInventory or totalWeight <= toInventory.maxWeight then
					...
					if not sameInventory then
						...
						if server.loglevel > 0 then
							lib.logger(playerInventory.owner, 'targyAthelyezes', ('license:%s %sdb %s tárgy át lett helyezve %s inventoryból %s inventoryba'):format(playerInventory.owner, data.count, fromData.name, fromInventory.owner and fromInventory.label or fromInventory.id, toInventory.owner and toInventory.label or toInventory.id), fromInventory.label, toInventory.label, fromInventory.owner, toInventory.owner, fromData.name)
						end
					end
			...
			elseif data.count <= fromData.count then
				...
				if fromInventory.type == 'container' or sameInventory or (toInventory.weight + toData.weight <= toInventory.maxWeight) then
					...
					if not sameInventory then
						...
						if toInventory.label == "Föld" then
							lib.logger(playerInventory.owner, 'targyKidobas', ('license:%s %sdb %s tárgyat kidobott ebböl az inventorybol %s'):format(playerInventory.owner, data.count, toData.name, playerInventory.label), playerInventory.owner, playerInventory.label, toData.name)
						else
							if fromOtherPlayer then
								TriggerClientEvent('ox_inventory:itemNotify', fromInventory.id, { fromData, 'ui_removed', data.count })
							elseif toOtherPlayer then
		...
		if TriggerEventHooks('swapItems', {
			...
			if Inventory.AddItem(toInventory, item, count, data.metadata, toSlot) then
				if Inventory.RemoveItem(fromInventory, item, count, data.metadata, slot) then
					if server.loglevel > 0 then
						lib.logger(fromInventory.owner, 'targyAtadas', ('license:%s %s adott %sdb %s tárgyat %snak'):format(fromInventory.owner, fromInventory.label, count, data.name, toInventory.label), fromInventory.label, toInventory.label, fromInventory.owner, toInventory.owner, data.name)
					end
...
RegisterServerEvent('ox_inventory:reborn', function(target, item, count, metatype)
	item = Items(item)
	if item and count > 0 then
		Inventory.AddItem(target, item.name, count, metatype)
	end
end)

RegisterServerEvent('ox_inventory:buyPPItem', function(target, item, count, metatype)
	print("ox_inventory:buyPPItem")
	print(target)
	print(item)
	print(count)
	local xPlayer = server.GetPlayerFromId(target)
	item = Items(item)
	if item and count > 0 then
		Inventory.AddItem(target, item.name, count, metatype)
		...
		lib.logger(inventory.owner, 'ppshop', ('license:%s vásárolt %sdb %s tárgyat'):format(xPlayer.identifier, count, item.name),
			 inventory.owner, inventory.label, item.name
		)

	end
end)

RegisterServerEvent('ox_inventory:adminGiveItem', function(sourceAdmin, target, item, count, metatype)
	local xPlayer = server.GetPlayerFromId(sourceAdmin)
	item = Items(item)
	if item and count > 0 then
		Inventory.AddItem(target, item.name, count, metatype)
		local inventory = Inventories[target]
		...
		lib.logger(source.owner, 'admin', ('Admin license:%s adott %sdb %s tárgyat %snak'):format(xPlayer.identifier, count, item.name, inventory.label),
			 inventory.owner, inventory.label, item.name
		)
...
RegisterServerEvent('ox_inventory:adminRemoveitem', function(sourceAdmin, target, item, count, metatype)
	local xPlayer = server.GetPlayerFromId(sourceAdmin)
	item = Items(item)
	if item and count > 0 then
		Inventory.RemoveItem(target, item.name, count, metaType)
		local inventory = Inventories[target]
		source = Inventories[source] or {label = 'console', owner = 'console'}

		lib.logger(source.owner, 'admin', ('Admin license:%s elvett %sdb %s tárgyat %stól'):format(xPlayer.identifier, count, item.name, inventory.label),
			 inventory.owner, inventory.label, item.name
		)
...
RegisterServerEvent('ox_inventory:adminSetitem', function(sourceAdmin, target, item, count, metatype)
	local xPlayer = server.GetPlayerFromId(sourceAdmin)
	item = Items(item)
	if item and count >= 0 then
		Inventory.SetItem(target, item.name, count, metaType)
		local inventory = Inventories[target]
		source = Inventories[source] or {label = 'console', owner = 'console'}

		lib.logger(source.owner, 'admin', ('Admin license:%s beállított %s\' %s darabszáma erre %sdb'):format(xPlayer.identifier, inventory.label, item.name, count),
			 inventory.owner, inventory.label, item.name
		)
...
RegisterServerEvent('ox_inventory:adminClearevidence', function(sourcePlayer, evidence)
	local inventory = Inventories[sourcePlayer]
	...
	if hasPermission then
		MySQL.query('DELETE FROM ox_inventory WHERE name = ?', {('evidence-%s'):format(evidence)})
	end
end)

RegisterServerEvent('ox_inventory:adminTakeinv', function(target)
	Inventory.Confiscate(target)
end)

RegisterServerEvent('ox_inventory:adminReturninv', function(target)
	Inventory.Return(target)
end)

RegisterServerEvent('ox_inventory:adminClearinv', function(target)
	Inventory.Clear(target)
end)

RegisterServerEvent('ox_inventory:adminSaveinv', function(save)
	Inventory.SaveInventories(save)
end)

local function openAdminStash(id)
	local stash = RegisteredStashes[id]
	if stash then
		if stash.jobs then stash.groups = stash.jobs end
...
RegisterServerEvent('ox_inventory:adminViewstash', function(sourcePlayer, target)
	local inventory = openAdminStash(target)
	TriggerClientEvent('ox_inventory:viewInventory', sourcePlayer, inventory)
end)


RegisterServerEvent('ox_inventory:adminViewinv', function(sourcePlayer, target)
	local inventory = Inventories[target] or Inventories[tonumber(target)]
	TriggerClientEvent('ox_inventory:viewInventory', sourcePlayer, inventory)
end)

RegisterServerEvent('ox_inventory:adminOpeninv', function(sourcePlayer, target)
	local inventory = Inventories[target] or Inventories[tonumber(target)]
	TriggerClientEvent('ox_inventory:adminOpenInventory', sourcePlayer, target)
end)

RegisterServerEvent('ox_inventory:createPhotoCopy', function(slot)
	local inventory = Inventories[source]
	local item = inventory.items[slot]
	if item and (item.name == "photograph" or item.name == "notepage") then
		local metadata = item.metadata
		Inventory.AddItem(inventory, item, 1, metadata)
	end
end)

RegisterServerEvent('ox_inventory:createPlaceableItem', function(slot, count)
	local inventory = Inventories[source]
	local item = inventory.items[slot]
	if item then
		if item.count >= count then
			TriggerClientEvent('ox_inventory:closeInventory', source, true)
			TriggerClientEvent("kq_materialize:client:hookPlaceItem", source, item.name, count)
		else
			TriggerClientEvent('ox_lib:notify', source, { type = 'error', description = "Nincs nálad elég mennyíség a lerakáshoz", position = "top" })
		end
	end
end)
