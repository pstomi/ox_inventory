...
local function canAffordItem(inv, currency, price)
	...
	local canAfford = price >= 0 and xPlayer.getAccount("bank").money >= price
			...
			if toData == nil or (fromItem.name == toItem?.name and fromItem.stack and table.matches(toData.metadata, metadata)) then
				...
				if server.loglevel > 0 then
					if server.loglevel > 1 or fromData.price >= 500 then
						local shopname = {}
						for str in string.gmatch(shop.label, "%S+") do
							table.insert(shopname, str)
						end
						lib.logger(playerInv.owner, 'targyVasarlas', ('%s %s %s a %s-ból'):format("license:"..playerInv.owner, playerInv.label, message:lower(), shop.label),
							 shop.label, shopname[1]
						)
					end
				end
