...
shared = {
    ...
    police = json.decode(GetConvar('inventory:police', '["police", "sheriff", "fbi", "atf", "rangers"]')),
	admin = json.decode(GetConvar('inventory:admin', '["admin"]')),
	ambulance = json.decode(GetConvar('inventory:ambulance', '["ambulance", "ces"]')),
	mechanics = json.decode(GetConvar('inventory:mechanics', '["mechanics"]')),
...
do
	if type(shared.admin) == 'string' then
		shared.admin = {shared.admin}
	end

	local admin = table.create(0, #shared.admin)

	for i = 1, #shared.admin do
		admin[shared.admin[i]] = 0
	end
	shared.admin = admin
end
...
do
	...
	local ambulance = table.create(0, #shared.ambulance)
...
do
	...
	for i = 1, #shared.mechanics do
		mechanics[shared.mechanics[i]] = 0
	end
...
if IsDuplicityVersion() then
    server = {
        ...
        randomloot = GetConvarInt('inventory:randomloot', 0) == 1,
